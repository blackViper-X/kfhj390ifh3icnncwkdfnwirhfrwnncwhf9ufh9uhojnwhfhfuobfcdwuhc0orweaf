...
async function exchangeCodeForTokens(config) {
  ...
  if (!accessToken) {
    throw new Error(
      `Google did not return an access_token during code exchange. Response: ${JSON.stringify(response.data)}`
    );
  }
...
async function refreshAccessToken(config) {
  ...
  if (!accessToken) {
    throw new Error(
      `Google did not return an access_token during refresh. Response: ${JSON.stringify(response.data)}`
    );
  }
