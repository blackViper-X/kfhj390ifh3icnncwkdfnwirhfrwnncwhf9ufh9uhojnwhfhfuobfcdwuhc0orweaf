...
async function exchangeCodeForTokens(config) {
  ...
  if (!shortLivedToken) {
    throw new Error(
      `Instagram/Facebook did not return an access_token during code exchange. Response: ${JSON.stringify(response.data)}`
    );
  }
  ...
  if (!accessToken) {
    throw new Error(
      `Instagram/Facebook did not return a long-lived access_token. Response: ${JSON.stringify(longLivedResponse.data)}`
    );
  }
...
async function refreshAccessToken(config) {
  ...
  if (!newAccessToken) {
    throw new Error(
      `Instagram/Facebook did not return an access_token during refresh. Response: ${JSON.stringify(response.data)}`
    );
  }
...
/**
 * Instagram Graph accounts are always reached through the linked Facebook
 * Page, so account discovery mirrors the Facebook adapter: read /me and
 * expand each page's instagram_business_account.
 * The OAuth callback relies on this being present.
 */
