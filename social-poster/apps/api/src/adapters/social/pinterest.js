...
async function exchangeCodeForTokens(config) {
  ...
  if (!accessToken) {
    throw new Error(
      `Pinterest did not return an access_token during code exchange. Response: ${JSON.stringify(response.data)}`
    );
  }
  ...
  return {
    ...
    expiresAt: new Date(Date.now() + (response.data.expires_in || 2592000) * 1000),
  };
}
...
async function refreshAccessToken(config) {
  ...
  if (!accessToken) {
    throw new Error(
      `Pinterest did not return an access_token during refresh. Response: ${JSON.stringify(response.data)}`
    );
  }
